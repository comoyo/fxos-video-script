...
function getExchangeRate() {
  ...
  x.onload = function() {
    if (x.status === 200) {
      _rate = Number(JSON.parse(x.responseText).query.results.rate.Rate);
      
      localStorage.setItem('rate', _rate);
    }
    else {
      _rate = Number(localStorage.getItem('rate'));
    }
  };
  ...
  x.open('GET', 'http://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20yahoo.finance.xchange%20where%20pair%20in%20%28%22NOKEUR%22%29&env=store://datatables.org/alltableswithkeys&format=json');
  x.send();
}
